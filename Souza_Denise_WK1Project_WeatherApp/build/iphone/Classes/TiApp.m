...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kSouza_Denise_WK1Project_WeatherAppUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kSouza_Denise_WK1Project_WeatherAppUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Souza_Denise_WK1Project_WeatherApp app using the node.js CLI
