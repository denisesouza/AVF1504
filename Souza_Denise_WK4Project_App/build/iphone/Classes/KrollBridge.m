...
NSString * Souza_Denise_WK4Project_App$ModuleRequireFormat = @"(function(exports){"
...
@implementation Souza_Denise_WK4Project_AppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_souza_denise_wk4project_app);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_souza_denise_wk4project_app gc];
}
...
{
	// create Souza_Denise_WK4Project_App global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Souza_Denise_WK4Project_App" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_souza_denise_wk4project_app = [[Souza_Denise_WK4Project_AppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_souza_denise_wk4project_app];
	
	NSString *_souza_denise_wk4project_appNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _souza_denise_wk4project_appNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_souza_denise_wk4project_app valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Souza_Denise_WK4Project_App$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.souza_denise_wk4project_app.kroll" 
