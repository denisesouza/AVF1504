...
NSString * Souza_Denise_WK2Project_WeatherApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation Souza_Denise_WK2Project_WeatherAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_souza_denise_wk2project_weatherapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_souza_denise_wk2project_weatherapp gc];
}
...
{
	// create Souza_Denise_WK2Project_WeatherApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Souza_Denise_WK2Project_WeatherApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_souza_denise_wk2project_weatherapp = [[Souza_Denise_WK2Project_WeatherAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_souza_denise_wk2project_weatherapp];
	
	NSString *_souza_denise_wk2project_weatherappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _souza_denise_wk2project_weatherappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_souza_denise_wk2project_weatherapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Souza_Denise_WK2Project_WeatherApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.souza_denise_wk2project_weatherapp.kroll" 
